/**
 * Helper for generating strings that resemble the raw description strings that come from rescuegroups.
 * Getters for boilerplate, upcoming, and bio sections return string arrays wrapped in p tags, while
 * the corresponding setFoo methods don't require p tag wrapping and will take care of that for you.
 */
export class RescueGroupsV2AnimalRawDescriptionBuilder {
  private static wrapInP(strs: string[]): string[] {
    return strs.map(s => `<p>${s}</p>`);
  }

  private _boilerplate = [
  ...
  public get boilerplate(): string[] {
    return this._boilerplate;
  }
  public setBoilerplate(boilerplate: string[]) {
    this._boilerplate = RescueGroupsV2AnimalRawDescriptionBuilder.wrapInP(boilerplate);
  }
  ...
  public set age(age: string) {
    this.bio[0] = `<p>Astro is a ${age} Turkish Van Mix who was one of many cats we rescued from a hoarding situation.</p>`
  }
